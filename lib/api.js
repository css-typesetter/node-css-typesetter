const Doc = require('./doc')
const DB = require('./db')
const PDFGen = require('./generator')
const _ = require('underscore')

module.exports = (api, address) => {
  //
  api.get('/:doc/page(:pagenum).html', (req, res, next) => {
    Doc.genPage(req.params.doc, `page${req.params.pagenum}.html`).then(doc => {
      res.send(doc)
    }).catch(next)
  })

  api.get('/:doc/index.html', (req, res, next) => {
    Doc.genPage(req.params.doc, null).then(doc => {
      res.send(doc)
    }).catch(next)
  })

  api.get('/:doc/render.pdf', (req, res, next) => {
    res.header('Content-Type', 'application/pdf')
    PDFGen(req.params.doc, res).then(() => res.end()).catch(next)
  })

  api.get('/:doc/static/*', (req, res, next) => {
    DB.getContent(req.params.doc, req.path).then(content => {
      res.header('Content-Type', 'text/css')
      res.send(content)
    }).catch(next)
  })

  api.get('/:doc', (req, res, next) => {
    Promise.all([
      Doc.readContext(req.params.doc).then(ctx => {
        return _.map(ctx, (val, key) => ({val, key}))
      }),
      Doc.getAllPagesContent(req.params.doc)
    ]).then(fetched => {
      return res.json(fetched)
    }).catch(next)
  })

  // modification API

  api.post('/:doc', (req, res, next) => {
    DB.createDir(req.params.doc).then(() => res.end()).catch(next)
  })

  api.post('/:doc/page', (req, res, next) => {
    DB.createFile(req.params.doc, 'page1', req.body)
    .then(() => res.end())
    .catch(next)
  })
  api.put('/:doc/page/:num', (req, res, next) => {
    DB.updateFile(req.params.doc, `page${req.params.num}`, req.body)
    .then(() => res.end())
    .catch(next)
  })

  api.put('/:doc/context', (req, res, next) => {
    const ctx = _.reduce(req.body, (acc, i) => {
      acc[i.key] = i.val
      return acc
    }, {})
    DB.updateFile(req.params.doc, 'context.json', JSON.stringify(ctx))
    .then(() => res.end()).catch(next)
  })
}
